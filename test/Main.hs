...
worldLineTests :: (String, Geodetic, Geodetic, Length Double, Dimensionless Double, Dimensionless Double) -> Test
...
pointTest :: (String, Geodetic, Geodetic) -> Test
...
prop_grid1 :: GridPoint GridTM -> GridOffset -> Bool
...
ukSampleGrid :: [(String, GridPoint UkNationalGrid, Geodetic, String)]
...
ukGridTest5 (_, gp, geo, testName) = testCase testName $ HU.assertBool ""
   $ offsetDistance (gridOffset gp $ toGrid UkNationalGrid $ toLocal _OSGB36 geo) < 1 *~ meter
...
prop_rayPath1 :: Ray -> Bool
prop_rayPath1 r@(Ray pt b e) = 
      samePlace pt pt1 && sameAngle b b1 && sameAngle e e1
   where (pt1,b1,e1) = pathFunc (getRay r) _0


type ContinuityTest = Geodetic -> Bearing -> Azimuth -> Distance -> Distance -> Property

type ContinuityTest1 = Geodetic -> Bearing -> Distance2 -> Distance2 -> Property
...
prop_pathContinuity ::
   (Geodetic -> Angle Double -> Angle Double -> Path) -> ContinuityTest
...
prop_pathContinuity1 :: (Geodetic -> Angle Double -> Path) -> ContinuityTest1
...
prop_rayContinuity :: ContinuityTest
...
prop_rhumbContinuity :: ContinuityTest1
